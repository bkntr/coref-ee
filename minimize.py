...
def handle_line(line, document_state, language, labels, stats):
  ...
  if len(document_state.text) == 0 and len(line.strip()) == 0:
    return None
...
def minimize_partition(name, language, extension, labels, stats):
  ...
  minimize_file(input_path, language, labels, stats, output_path)

def minimize_file(input_path, language, labels, stats, output_path=None):
  if output_path is None:
    output_path = "{}.jsonlines".format(input_path)
...
def main():
  ...
  if args.input:
    minimize_file(args.input, "english", labels, stats)
    return
